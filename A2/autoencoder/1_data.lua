...
function getdata(datafile, inputsize, std)
   local rawdata = torch.load(datafile)
   local data,labels = parseDataLabel(rawdata.data, 100000, 3, 96, 96)
   local dataset ={}
   ...
   local dsample = torch.Tensor(nchannels, inputsize*inputsize)

   function dataset:conv()
      dsample = torch.Tensor(nchannels,inputsize,inputsize)
   end
