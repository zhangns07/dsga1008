...
--                max_epoch=4,  -- when to start decaying learning rate
                max_epoch=2,  -- when to start decaying learning rate
...
local function grucell(input, prevh)
...
while epoch < params.max_max_epoch do
    ...
    if step % torch.round(epoch_size / 10) == 10 then
    	...
    	torch.save('model.GRU.net',model)
    end
...
-- torch.save('model.GRU.net',model)
-- torch.save('vocab_map.t7',ptb.vocab_map)
-- torch.save('vocab_invmap.t7',ptb.vocab_invmap)
